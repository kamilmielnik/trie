**@kamilmielnik/trie** • **Docs**

***

# @kamilmielnik/trie

## Classes

- [Trie](classes/Trie.md)

## Interfaces

- [Node](interfaces/Node.md)

## Type Aliases

- [Descendant](type-aliases/Descendant.md)
- [TraverseCallback](type-aliases/TraverseCallback.md)
- [TraverseOptions](type-aliases/TraverseOptions.md)

## Variables

- [CLOSE\_PARENS](variables/CLOSE_PARENS.md)
- [OPEN\_PARENS](variables/OPEN_PARENS.md)

## Functions

- [add](functions/add.md)
- [deserialize](functions/deserialize.md)
- [find](functions/find.md)
- [fromArray](functions/fromArray.md)
- [has](functions/has.md)
- [hasPrefix](functions/hasPrefix.md)
- [nodeKeyComparator](functions/nodeKeyComparator.md)
- [remove](functions/remove.md)
- [serialize](functions/serialize.md)
- [toArray](functions/toArray.md)
- [traverse](functions/traverse.md)
